...
import json
import faapi
from yippi import YippiClient, Rating


def main():
    config = json.load(open("config.json", "r"), object_hook=lambda d: SimpleNamespace(**d))
    config.headers = { "User-Agent": f"{config.client_name}/{config.client_version}" }

    url_lists = sys.argv[1:]
    if len(url_lists) == 0:
        print(f"Usage: {sys.argv[0]} URL_LIST_FILE [additional URL list files)")
        exit()

    # read in all e6 and FA urls from files specified via command-line arguments
    e6_ids = []
    fa_ids = []
    ...
    e6_client = YippiClient(config.client_name, config.client_version, config.e6.username)

    for post_id in e6_ids:
        dl_done += 1
        print(f"{dl_done : >{len(str(dl_total))}} / {dl_total}: ", end="", flush=True)
...
def gather_post_ids(filename):
    ...
    with open(filename, "r") as f:
        ...
        for l in lines:
            if "e621.net/post/show" in l or "e621.net/posts/" in l or "e926.net/post/show" in l or "e926.net/posts/" in l:
                # truncate first the left, then the right part of the URL
                try:
                    postID = int(l.rsplit("/", 1)[1].split("?")[0])
                # old URL format with "/[truncated tags]" at the end
                except:
                    postID = int(l.rsplit("/", 2)[1].split("/")[0])
                
                e6_ids.append(postID)
            elif "furaffinity.net/view/" in l or "furaffinity.net/full/" in l:
                # truncate first the left, then the right part of the URL,
                # slightly differently depending on whether it ends in a slash or not
                if l.endswith("/"):
                    postID = int(l.rsplit("/", 2)[1].split("/")[0])
                else:
                    postID = int(l.rsplit("/", 1)[1])
...
def e6_download(post_id, client, config):
    ...
    artists = filter(lambda a: a not in ["conditional_dnp", "avoid_posting"],
                     post.tags["artist"])

    # canonicalise author(s)
    filename = canonicalise(f"{id}.{ext}", config)
    subdir = canonicalise(", ".join(
        [artist.replace("_", " ")
               .replace(" (artist)", "")
               .replace(" (fa)", "")
               .title()
